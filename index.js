...
function isMobile () {
  let isMobile = false
  const regExp1 = /(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m([...]
  const regExp2 = /1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi([...]
...
var paymentConfig = window.PAYMENT_CONFIG || {};
var portalLoginHandler = null;
...
function enableLoginButton() {
    ...
    if (loginButton) {
        ...
        loginButton.innerHTML = globalConfig.buttonText || "Log In";
    }
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    loginButton.addEventListener("click", function () {
        if (portalLoginHandler) {
            portalLoginHandler();
        } else {
            showOperHint("Preparing Wi-Fi login. Please wait a moment.");
        }
    });
});
...
Ajax.post(
    ...
    function (res) {
        ...
        function formatPlanAmount(plan) {
            var currency = paymentConfig.currency || "GHS";
            var amount = Number(plan.amount || 0) / 100;
            return currency + " " + amount.toFixed(2).replace(".00", "");
        }

        function renderPaymentPackages() {
            var packageContainer = document.getElementById("payment-packages");
            var plans = paymentConfig.plans || [];
            ...
            for (var i = 0; i < plans.length; i++) {
                ...
                packageItem.className = "payment-package";
                name.className = "package-name";
                price.className = "package-price";
                button.className = "package-pay";
                button.type = "button";
                button.setAttribute("data-plan-id", plans[i].id);

                name.textContent = plans[i].name || plans[i].label;
                price.textContent = formatPlanAmount(plans[i]);
                button.textContent = "Pay MoMo";

                details.appendChild(name);
                details.appendChild(price);
                packageItem.appendChild(details);
                packageItem.appendChild(button);
        ...
        function verifyPaymentAndLogin(transaction, plan) {
            if (!paymentConfig.verifyEndpoint) {
                showOperHint("Payment received. Add your verification endpoint in payment-config.js so a voucher can be issued automatically.");
                enableLoginButton();
                return;
            }

            showOperHint("Confirming payment...");
            Ajax.post(paymentConfig.verifyEndpoint,
                JSON.stringify({
                ...
                function (response) {
                    var verified = JSON.parse(response);
                    var voucherCode = verified.voucherCode || verified.result && verified.result.voucherCode;
                    if (voucherCode) {
                        document.getElementById("voucherCode").value = voucherCode;
                        handleSubmit();
                    } else {
                        showOperHint(verified.message || "Payment confirmed, but no voucher code was returned.");
                        enableLoginButton();
                    }
                }
            );
        ...
        function handlePayment(planId) {
            var plan = getPaymentPlan(planId);
            var phone = document.getElementById("payment-phone").value.trim();
            var enteredEmail = document.getElementById("payment-email").value.trim();
            var emailSafePhone = phone.replace(/[^0-9]/g, "") || "customer";
            var email = enteredEmail || ("wifi-" + emailSafePhone + "@galaxy-link-wifi.onrender.com");

            if (!plan) {
                showOperHint("Please add at least one Wi-Fi package in payment-config.js.");
                return;
            }
            if (!paymentConfig.publicKey || paymentConfig.publicKey.indexOf("REPLACE_WITH") !== -1) {
                showOperHint("Add your Paystack public key in payment-config.js.");
                return;
            }
            if (!phone) {
                showOperHint("Enter your Mobile Money number.");
                return;
            }
            if (!window.Paystack && !window.PaystackPop) {
                showOperHint("Paystack checkout could not load. Allow js.paystack.co before Wi-Fi login.");
                return;
            }

            showOperHint("Opening secure Paystack checkout...");
            var paymentOptions = {
                key: paymentConfig.publicKey,
                email: email,
                amount: plan.amount,
                currency: paymentConfig.currency || "GHS",
                phone: phone,
                channels: ["mobile_money"],
                label: paymentConfig.businessName || "Wi-Fi Access",
                metadata: {
                    custom_fields: [
                        {
                        ...
                        {
                            display_name: "Wi-Fi Package",
                            variable_name: "wifi_package",
                            value: plan.label
                        },
                        {
                ...
                onSuccess: function (transaction) {
                    verifyPaymentAndLogin(transaction, plan);
                },
                onCancel: function () {
                    showOperHint("Payment was cancelled.");
                    enableLoginButton();
                },
                onError: function (error) {
                    showOperHint(error && error.message ? error.message : "Payment could not be started.");
                    enableLoginButton();
                }
            };

            if (window.Paystack) {
                var popup = new Paystack();
                ...
                PaystackPop.setup({
                    ...
                    callback: function (response) {
                        verifyPaymentAndLogin({ reference: response.reference }, plan);
                    },
                    onClose: function () {
                        showOperHint("Payment was cancelled.");
                        enableLoginButton();
        ...
        document.getElementById("payment-packages").addEventListener("click", function (event) {
            var target = event.target;
            if (target.classList.contains("package-pay")) {
                handlePayment(target.getAttribute("data-plan-id"));
            }
        });
